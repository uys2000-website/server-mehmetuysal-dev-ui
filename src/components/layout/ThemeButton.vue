<template>
  <div class="dropdown dropdown-top dropdown-start lg:dropdown-bottom lg:dropdown-end m-0 p-0">
    <div tabindex="0" role="button" class="btn h-9 m-0 p-0 px-2 py-3 min-h-0 bg-transparent border-none">{{ theme }}
    </div>
    <ul tabindex="0"
      ...
      <template v-for="(theme, index) in themes" :key="index">
        <li>
          <button :data-theme="theme" class="btn flex flex-row items-center w-full px-2 cursor-pointer flex-nowrap"
            ...
            <span class="inline w-full flex-shrink">{{ theme }}</span>
